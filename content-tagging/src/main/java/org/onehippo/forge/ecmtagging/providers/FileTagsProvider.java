...
/**
 * This provider searches for files embeded in the document. This can be (local) links or images for instance.
 *
 * @todo Make the location of the facetselect configurable.
 */
public class FileTagsProvider extends AbstractTagsProvider {
    private static final Logger log = LoggerFactory.getLogger(FileTagsProvider.class);
            ...
            for (NodeIterator i = html.getNodes(); i.hasNext(); ) {
                ...
                if (file.isNodeType(HippoNodeType.NT_FACETSELECT)) {
                    ...
                    while (ni.hasNext()) {
                        ...
                        if (subFile.isNodeType(TaggingNodeType.NT_TAGGABLE) && subFile.hasProperty(
