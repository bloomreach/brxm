...
/**
 * A Plugin that exposes the tagsuggesting service within the Hippo ECM application. It coordinates all the
 * communication with the tag providers end returns the final TagCollection to the caller / frontend.
 */
public class TagSuggestor extends Plugin implements IClusterable {
    private static final Logger log = LoggerFactory.getLogger(TagSuggestor.class);

    /**
     * The servicename under which the calling resources can retrieve the TagSuggestor
     *
    ...
    /**
     * Include present tags: if true then the tags present in the document are also shown as suggestions. Otherwise they
     * are filtered from the suggestion list.
     */
    ...
    /**
     * Will search for tags for the given document. It will do this by passing it on to all the registered TagProviders
     * and combining the results into one TagCollection.
     *
     * @param nodeModel
     * @return
