...
Ext.ux.tot2ivn.VrTabPanel = Ext.extend(Ext.Panel,  {
    ...
    readTabs : function(removeExisting){
        ...
        tabs = this.el.query(this.autoTabSelector);
    ...
    onBeforeAdd : function(item) {
        var existing, es;
        existing = item.events ? (this.items.containsKey(item.getItemId()) ? item : null) : this.items.get(item);
        if (existing) {
            this.setActiveTab(item);
            return false;
        }
...
//        }
    ...
    autoSizeTabs : function() {
        ...
        each = Math.max(Math.min(Math.floor((aw-4) / count) - this.tabMargin, this.tabWidth), this.minTabWidth); // -4 for float errors in IE
        this.lastTabWidth = each;
        lis = this.strip.query('li:not(.x-tab-edge)');
    ...
    createScrollers : function() {
        ...
        // bottom
        sr = this.stripWrap.insertSibling({
    ...
    onWheel : function(e) {
        var d, pos, newpos, sw, s;
        d = e.getWheelDelta()*this.wheelIncrement*-1;
        e.stopEvent();
