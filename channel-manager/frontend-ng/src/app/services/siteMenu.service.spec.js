...
describe('SiteMenuService', () => {
  ...
  describe('emptyMenu', () => {
    beforeEach(() => {
      ...
      HstService.doGet.and.returnValue($q.when({ data: emptyMenu }));

      // preload the empty menu
      $rootScope.$apply(() => SiteMenuService.loadMenu('emptyMenu'));
      HstService.doGet.calls.reset();
    });

    it('successfully loads a empty menu', (done) => {
      SiteMenuService.loadMenu('emptyMenu')
        .then((menu) => {
          expect(menu.id).toBe(emptyMenu.id);
          expect(menu.items.length).toBe(0);
          expect(menu.prototypeItem.id).toBe('pItem');
          done();
        })
        .catch(() => fail());
      expect(HstService.doGet).toHaveBeenCalledWith('emptyMenu');
      $rootScope.$digest();
    });
  });

  describe('nonEmptyMenu', () => {
    beforeEach(() => {
      spyOn(HstService, 'doGet');
