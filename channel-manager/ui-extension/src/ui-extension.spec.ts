...
describe('register', () => {
  ...
  it('uses the Promise constructor provided in the configuration', (done) => {
    const fakePromise = ({} as PromiseConstructor);

    UiExtension.register({ Promise: fakePromise })
      .then(() => {
        expect(Penpal.Promise).toBe(fakePromise);
        done();
      });
  ...
  describe('on failure', () => {
    it('rejects with error code "NotInIframe" when there is no parent', (done) => {
      Penpal.connectToParent = () => {
        // as generated by penpal
        const error = new Error('connectToParent() must be called within an iframe');
        (error as any).code = Penpal.ERR_NOT_IN_IFRAME;
        throw error;
      };
    ...
    it('rejects with error code "IncompatibleParent" when the parent does not implement getProperties()', (done) => {
    ...
    it('rejects with error code "ConnectionDestroyed" when the parent connection is destroyed', (done) => {
      const destroyedParent = {
        getProperties: () => {
          // as generated by penpal
          const error = new Error('Unable to send getProperties() call due to destroyed connection');
          (error as any).code = Penpal.ERR_CONNECTION_DESTROYED;

          console.log('throw error code', (error as any).code);
          throw error;
        },
      };
    ...
    it('rejects with error code "InternalError" when parent.getProperties rejects', (done) => {
      const errorParent = {
