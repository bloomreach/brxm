...
public class FacetedNavigationFilteredTest extends AbstractDateFacetNavigationTest {
    ...
    @Test
    public void test_incorrect_facnav_filter_doesnt_break_node() throws Exception {
        ...
        facetNavigation.setProperty(FacNavNodeType.HIPPOFACNAV_FACETS, new String[]{"hippo:date$year", "hippo:date$month"});
        facetNavigation.setProperty(FacNavNodeType.HIPPOFACNAV_FACETNODENAMES, new String[]{"year", "month"});
        // invalid filter 'hippo:brand='
        facetNavigation.setProperty(FacNavNodeType.HIPPOFACNAV_FILTERS, new String[]{"hippo:brand="});

        session.save();

        facetNavigation = session.getRootNode().getNode("test/facetnavigation/hippo:navigation");
        assertEquals("Broken fac nav node should have no children but be accessible",0L,
                facetNavigation.getNodes().getSize());
        ...
        facetNavigation = session.getRootNode().getNode("test/facetnavigation/hippo:navigation");
        assertTrue( "Broken fac nav node can be repaired again",
                facetNavigation.getNodes().getSize() > 0);
    }
