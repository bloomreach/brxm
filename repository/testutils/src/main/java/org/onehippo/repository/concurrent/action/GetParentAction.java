...
public class GetParentAction extends Action {
    
    public GetParentAction(ActionContext context) {
        super(context);
    }
    
    @Override
    public boolean canOperateOnNode(Node node) throws RepositoryException {
        return !node.getPath().equals(context.getDocumentBasePath()) && !node.getPath().equals(context.getAssetBasePath());
    }
