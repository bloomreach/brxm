...
public interface Workflow extends Remote, Serializable {
     ...
     * By convention, keys that are names or signatures of methods implemented by the workflow provide information to the application
     * program whether the workflow method is available this time, or will result in a WorkflowException.  The value for these keys
     * will often be a {@link java.lang.Boolean} to indicate the enabled status of the method.<p/>
     * Non-standard keys in this map should be prefixed with the implementation package name using dot seperations.
     * @return a map containing hints given by the workflow, the data in this map may be considered valid until the document itself changes
     * @throws org.hippoecm.repository.api.WorkflowException thrown in case the implementing workflow encounters an error, this exception should normally never be thrown by implementations for the hints method.
