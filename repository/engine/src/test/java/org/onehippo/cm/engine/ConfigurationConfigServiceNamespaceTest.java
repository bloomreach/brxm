...
public class ConfigurationConfigServiceNamespaceTest extends BaseConfigurationConfigServiceTest {
    ...
    public void expect_namespace_change_to_abort_with_exception() throws Exception {
        ...
        try {
            ...
            assertEquals("Failed to process namespace definition defined in core/test-group/test-project/test-module-0 [config: string]: " +
    ...
    public void expect_cnd_reloads_to_work() throws Exception {
        ...
        try (Log4jInterceptor interceptor = Log4jInterceptor.onDebug().trap(ConfigurationConfigService.class).build()) {
            ...
            assertTrue(interceptor.messages().anyMatch(m -> m.equals("skipping CND already loaded in baseline: 'test2b.cnd' defined in core/test-group/test-project/test-module-0 [config: string].")));
        }
    }
    ...
    public void expect_debug_message_to_be_logged_upon_loading_of_cnd() throws Exception {
        ...
        try (Log4jInterceptor interceptor = Log4jInterceptor.onDebug().trap(ConfigurationConfigService.class).build()) {
            applyDefinitions(source);
            assertTrue(interceptor.messages().anyMatch(m -> m.equals("processing CND 'test3.cnd' defined in core/test-group/test-project/test-module-0 [config: string].")));
        }
    ...
    public void expect_parse_error_in_cnd() throws Exception {
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("Failed to parse cnd 'unknown.cnd' (core/test-group/test-project/test-module-0 [config: string])"));
        }
    }
