...
import org.hippoecm.frontend.sa.plugins.standardworkflow.types.JcrTypeModel;
...
import org.hippoecm.frontend.sa.template.plugins.field.NodeFieldPlugin;
...
public class TemplateListPlugin extends RenderPlugin {
    ...
    public TemplateListPlugin(IPluginContext context, IPluginConfig config) {
        ...
        try {
            ...
            while (templates.hasNext()) {
                ...
                if (template.isNodeType("frontend:plugincluster")) {
                    ...
                    typeModel = typeStore.getTypeModel(typeNode.getName());
                    break;
                }
            }
        ...
        List<TypeDescriptor> templateList = new LinkedList<TypeDescriptor>();
        ...
        if (engine != null) {
            ...
            if (templates != null) {
                for (String type : templates) {
                    TypeDescriptor typeDescriptor = engine.getType(type);
        ...
        AbstractView templateView = new AbstractView("templates", new ListDataProvider(templateList)) {
            ...
            public void populateItem(Item item) {
                final TypeDescriptor type = (TypeDescriptor) item.getModelObject();
    ...
    protected void addField(TypeDescriptor typeDescriptor) {
        try {
            String name = typeModel.addField(typeDescriptor.getName());
