...
public class ReviewedActionsIssueTest extends TestCase {
    ...
    public void testIssue688() throws WorkflowException, MappingException, RepositoryException, RemoteException {
        Node node, root = session.getRootNode();
        ...
        {  
            ...
            try {
                workflow.requestDeletion();
                assertTrue("Issue HREPTWO-688 is solved", true );
            } catch (WorkflowException e) {
                assertTrue("Issue HREPTWO-688 is still present", true );
            }
            session.save();
            session.refresh(true);
            //Utilities.dump(root.getNode("documents"));
