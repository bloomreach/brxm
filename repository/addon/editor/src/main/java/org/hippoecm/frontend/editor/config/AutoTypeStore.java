...
public class AutoTypeStore implements ITypeStore {
    ...
    public ITypeDescriptor getTypeDescriptor(String type) {
        AutoTypeDescriptor result = new AutoTypeDescriptor(type);
        if (result.isValid()) {
            return result;
        }
        return null;
    ...
    class AutoTypeDescriptor extends JavaTypeDescriptor {
        private static final long serialVersionUID = 1L;

        private String type;
        private transient NodeType nt = null;

        AutoTypeDescriptor(String type) {
            super(type, type);
        ...
        public Map<String, IFieldDescriptor> getFields() {
            ...
            if (nt != null) {
                ...
                for (NodeDefinition definition : childNodes) {
                    fields.put(definition.getName(), new AutoFieldDescriptor(definition.getName(), definition));
                }
    ...
    class AutoFieldDescriptor extends JavaFieldDescriptor {
        private static final long serialVersionUID = 1L;

        AutoFieldDescriptor(String path, ItemDefinition definition) {
            super(path);
