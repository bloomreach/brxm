...
public class ImageInfo {
    ...
    private static StringBuffer printLine(int indentLevels, String text, int value, int minValidValue, StringBuffer buf) {
        if (value >= minValidValue) {
            buf = printLine(indentLevels, text, Integer.toString(value), buf);
        }
        return buf;
    }

    private static StringBuffer printLine(int indentLevels, String text, String value, StringBuffer buf) {
    ...
    private static StringBuffer printVerbose(String sourceName, ImageInfo ii, StringBuffer buf) {
        buf = printLine(0, "File name:", sourceName, buf);
        buf = printLine(1, "File format: ", ii.getFormatName(), buf);
        buf = printLine(1, "MIME type: ", ii.getMimeType(), buf);
        buf = printLine(1, "Width (pixels): ", ii.getWidth(), 1, buf);
        buf = printLine(1, "Height (pixels): ", ii.getHeight(), 1, buf);
        return buf;
    }
    ...
    public static String analyse(String sourceName, InputStream in) {
        ...
        if (imageInfo.check()) {
            buf = printVerbose(sourceName, imageInfo, buf);
        }
        return buf.toString();
    }
