...
public class CKEditorImageService implements IDetachable {

    private static final long serialVersionUID = 1L;

    static final Logger log = LoggerFactory.getLogger(CKEditorImageService.class);

    private String editorId;
    private IRichTextImageFactory factory;

    public CKEditorImageService(IRichTextImageFactory factory, String editorId) {
    ...
    public String attach(JcrNodeModel model) {
        ...
        try {
            ...
            return "CKEDITOR.instances." + editorId + ".execCommand('insertImage', {"
                + CKEditorImage.URL + ": '" + item.getUrl() + "',"
                + CKEditorImage.FACET_SELECT + ": '" + item.getFacetSelectPath() + "'"
                + "});";
    ...
    public CKEditorImage createCKEditorImage(Map<String, String> p) {
        RichTextImage rti = loadImageItem(p);
        return new CKEditorImage(p, rti != null ? rti.getTarget() : null) {
            private static final long serialVersionUID = 1L;
    ...
    private RichTextImage loadImageItem(Map<String, String> values) {
        String path = values.get(CKEditorImage.FACET_SELECT);
