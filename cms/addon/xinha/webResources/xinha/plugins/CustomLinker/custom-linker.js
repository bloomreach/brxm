...
Xinha.loadStyle('dTree/dtree.css', 'CustomLinker');
...
CustomLinker.prototype._stripWicketAjax = function(value) {
    var CDataHead = '<![CDATA[';
    var CDataTail = ']]>';
    if(value.indexOf(CDataHead) > -1) {
      value = value.substring(value.indexOf(CDataHead) + CDataHead.length);
      if(value.indexOf(CDataTail) > -1) {
        value = value.substring(0, value.indexOf(CDataTail));
      }  
    }
...
CustomLinker.prototype.onGenerateOnce = function()
{
  this._dialog = new CustomLinker.Dialog(this);
};
// Inline Dialog for CustomLinker

CustomLinker.Dialog_dTrees = [ ];


CustomLinker.Dialog = function (linker)
{
  var  lDialog = this;
  this.Dialog_nxtid = 0;
  this.linker = linker;
  ...
  this.html   = false;
  this.dialog = false;
  this.callbackUrl = this.linker.editor.config.callbackUrl + '&browse=/';
...
CustomLinker.Dialog.prototype._prepareDialog = function()
{
  var lDialog = this;
  ...
  {
    Xinha._loadback(_editor_url + 'plugins/CustomLinker/dTree/dtree.js',
  ...
  if(this.files == false)
  {
    if(this.callbackUrl)
    {
      ...
      Xinha._postback(this.callbackUrl,
  ...
  if(this.html == false)
  {
    Xinha._getback(_editor_url + 'plugins/CustomLinker/dialog.html', function(txt) { lDialog.html = txt; lDialog._prepareDialog(); });
    return;
  }
  var html = this.html;

  // Now we have everything we need, so we can build the dialog.
  var dialog = this.dialog = new Xinha.Dialog(linker.editor, this.html, 'CustomLinker');
  var dTreeName = Xinha.uniq('dTree_');
    
  this.dTree = new dTree(dTreeName, _editor_url + 'plugins/CustomLinker/dTree/', linker, dialog);
  ...
  this.dialog.onresize = function()
    {
      if(dialog.getElementById('h1') != null) {
...
CustomLinker.Dialog.prototype.makeNodes = function(files, parent)
{
  for(var i = 0; i < files.length; i++)
  {
    if(typeof files[i] == 'string')
    {
      this.dTree.add(CustomLinker.nxtid++, parent,
                     files[i].replace(/^.*\//, ''),
...
CustomLinker.Dialog.prototype._lc = CustomLinker.prototype._lc;

CustomLinker.Dialog.prototype.show = function(inputs, ok, cancel)
{
  if(!this.ready)
  {
    var lDialog = this;
  ...
  // Show the dialog
  this.linker.editor.disableToolbar(['fullscreen','CustomLinker']);
...
CustomLinker.Dialog.prototype.hide = function()
{
