...
public class SaveDialog extends Dialog<Node> {
    private static final long serialVersionUID = 1L;
    private static final Logger log = LoggerFactory.getLogger(ContentImportDialog.class);

    public SaveDialog() {
        setTitle(Model.of("Save Session"));
        setSize(DialogConstants.MEDIUM);
        
        Component message;
        try {
            final HippoNode rootNode = (HippoNode) UserSession.get().getJcrSession().getRootNode();
            if (rootNode.getSession().hasPendingChanges()) {
                final StringBuilder buf = new StringBuilder("Pending changes:\n");
    ...
    public void onOk() {
        try {
            ...
            final HippoNode rootNode = (HippoNode) jcrSession.getRootNode();
            final StringBuilder buffer = new StringBuilder("User made changes at: ");
            appendPendingChangesFromNodeToBuffer(rootNode, buffer, ",");

            jcrSession.save();
            //only log when the save is successful
            logEvent(jcrSession.getUserID(), buffer.toString());
    ...
    private void appendPendingChangesFromNodeToBuffer(final HippoNode rootNode, final StringBuilder buf,
        ...
        final NodeIterator it = rootNode.pendingChanges();
        if (it.hasNext()) {
            while (it.hasNext()) {
    ...
    private void logEvent(String user, String message) {
        ...
        if (eventBus != null) {
            ...
            event.category("console").user(user).action("write-changes");
