    ...
    protected class LoginForm extends CaptchaForm {
        ...
        public LoginForm(final String id, final boolean autoComplete, final List<String> locales, final LoginHandler handler) {
            ...
            if (config.getBoolean(SHOW_TIMEZONES_CONFIG_PARAM)) {
                ...
                form.addLabelledComponent(new Label("timezone-label", new ResourceModel("timezone-label", "Time zone:")));
                form.add(timeZone);

            } else {
                form.addLabelledComponent(new Label("timezone-label").setVisible(false));
