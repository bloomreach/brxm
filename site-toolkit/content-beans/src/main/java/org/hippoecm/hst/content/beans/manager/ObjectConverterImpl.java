...
public class ObjectConverterImpl implements ObjectConverter {
    ...
    protected Map<String, Pair<String, ContentType>> dynamicBeanContentTypes = new HashMap<>();
    ...
    public ObjectConverterImpl(Map<String, Class<? extends HippoBean>> jcrPrimaryNodeTypeBeanPairs, String[] fallBackJcrNodeTypes) {
        ...
        dynamicBeanService = new HippoDynamicBeanService();
    }
        ...
        try {
            ...
            if (delegateeClass == null) {
                ...
                if (useNode.isNodeType(HippoNodeType.NT_DOCUMENT) && useNode.getParent().isNodeType(NT_HANDLE)) {
                    delegateeClass = createDynamicBean(node, null);
                } else {
    ...
    private Class<? extends HippoBean> createDynamicBean(final Node node, Class<? extends HippoBean> parentBean) throws RepositoryException {
        ...
        final Pair<String, ContentType> beanContentType = dynamicBeanContentTypes.get(beanType);

        if (parentBean == null) {
            parentBean = jcrBeanPrimaryNodeTypePairs.keySet()
                    .stream()
                    .filter(bean -> {
                            org.hippoecm.hst.content.beans.Node nodeAnnotation = bean
                                    .getDeclaredAnnotation(org.hippoecm.hst.content.beans.Node.class);
                            return nodeAnnotation != null && nodeAnnotation.jcrType() != null
                                    && nodeAnnotation.jcrType().endsWith(":basedocument");
                    })
                    .findFirst()
                    .orElse(null);
        }

        Class<? extends HippoBean> delegateeClass = dynamicBeanService.createDynamicDocumentBean(parentBean,
                beanContentType.getLeft(), beanContentType.getRight());

        if (delegateeClass != null) {
            this.jcrBeanPrimaryNodeTypePairs.put(delegateeClass, beanType);
            this.jcrPrimaryNodeTypeBeanPairs.put(beanType, delegateeClass);
        }
    ...
    /**
     * 
     * @param name of the document type to remove from the document type map
     */
    public void removeDynamicBean(String documentTypeName) {
        if (dynamicBeanContentTypes.containsKey(documentTypeName)) {
            Class<? extends HippoBean> beanlessClass = jcrPrimaryNodeTypeBeanPairs.get(documentTypeName);
            jcrPrimaryNodeTypeBeanPairs.remove(documentTypeName);
            jcrBeanPrimaryNodeTypePairs.remove(beanlessClass);
            dynamicBeanContentTypes.remove(documentTypeName);
        }
    }
     ...
     * @param contentType to add to the contentTypes map
     */
    public void addDynamicBeanContentType(String namespace, ContentType contentType){
        if (!dynamicBeanContentTypes.containsKey(contentType.getName())) {
            dynamicBeanContentTypes.put(contentType.getName(), Pair.of(namespace, contentType));
        }
    }
