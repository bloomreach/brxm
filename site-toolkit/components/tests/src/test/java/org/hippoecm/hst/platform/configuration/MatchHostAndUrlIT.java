...
public class MatchHostAndUrlIT extends AbstractBeanTestCase {
    ...
    @Test
    public void match_aboutfolder_results_in_index_sitemap_item_returned_with_relative_content_path_to_contact() {
        MockHttpServletResponse response = new MockHttpServletResponse();
        GenericHttpServletRequestWrapper containerRequest;
        {
            MockHttpServletRequest request = new MockHttpServletRequest();
            ...
            request.setServerName("localhost");
            request.addHeader("Host", "localhost");
            setRequestInfo(request, "/site", "/aboutfolder");
            containerRequest = new HstContainerRequestImpl(request, hstSitesManager.getPathSuffixDelimiter());
        }
        ...
        try {
            ...
            HstContainerURL hstContainerURL = hstURLFactory.getContainerURLProvider().parseURL(containerRequest, response, mount);
            ResolvedSiteMapItem resolvedSiteMapItem = mount.matchSiteMapItem(hstContainerURL.getPathInfo());
            assertEquals("Expected the _index_ sitemap item below the 'aboutfolder' sitemap item to be resolved",
                    "aboutfolder/_index_", resolvedSiteMapItem.getPathInfo());

            assertEquals("Expected the relative content path of the _index_ sitemap item pointing to 'contact' below the 'aboutfolder' folder",
                    "common/aboutfolder/contact", resolvedSiteMapItem.getRelativeContentPath());
    ...
    @Test
    public void index_sitemap_item_below_aboutfolder_relative_content_path_containing_property_placeholder() throws Exception {
    ...
    @Test
    public void match_aboutfolder_results_in_folder_sitemap_item_when_index_sitemap_item_points_to_non_existing_document() throws Exception {
        Session session = createSession();
        createHstConfigBackup(session);
        // point the _index_ sitemap item to non existing document
        session.getNode("/hst:hst/hst:configurations/unittestproject/hst:sitemap/aboutfolder/_index_")
        ...
        try {
            try {
                ...
                HstContainerURL hstContainerURL = hstURLFactory.getContainerURLProvider().parseURL(containerRequest, response, mount);
                ResolvedSiteMapItem resolvedSiteMapItem = mount.matchSiteMapItem(hstContainerURL.getPathInfo());
                assertEquals("Not Expected the _index_ sitemap item below the 'aboutfolder' item to be resolved because the document " +
                                "to which the '_index_' item points does not exist",
                        "aboutfolder", resolvedSiteMapItem.getPathInfo());
    ...
    @Test
    public void match_about_sub_folder_results_in_index_sitemap_returned_with_relative_content_path_to_address() {
        MockHttpServletResponse response = new MockHttpServletResponse();
        GenericHttpServletRequestWrapper containerRequest;
        {
            MockHttpServletRequest request = new MockHttpServletRequest();
            ...
            request.setServerName("localhost");
            request.addHeader("Host", "localhost");
            setRequestInfo(request, "/site", "/aboutfolder/aboutsubfolder");
            containerRequest = new HstContainerRequestImpl(request, hstSitesManager.getPathSuffixDelimiter());
        }
        ...
        try {
            ...
            HstContainerURL hstContainerURL = hstURLFactory.getContainerURLProvider().parseURL(containerRequest, response, mount);
            ResolvedSiteMapItem resolvedSiteMapItem = mount.matchSiteMapItem(hstContainerURL.getPathInfo());
            assertEquals("Expected the _index_ sitemap item below the 'aboutfolder/_default_' sitemap item to be resolved",
                    "aboutfolder/aboutsubfolder/_index_", resolvedSiteMapItem.getPathInfo());

            assertEquals("Expected the relative content path of the _index_ sitemap item pointing to 'address' below the 'aboutfolder/aboutsubfolder' folder",
                    "common/aboutfolder/aboutsubfolder/address", resolvedSiteMapItem.getRelativeContentPath());
    ...
    @Test
    public void index_sitemap_item_below_about_sub_folder_relative_content_path_containing_property_placeholder() throws Exception {
        Session session = createSession();
        createHstConfigBackup(session);
        ...
        try {
            try {
                ...
                assertEquals("Expected the relative content path of the _index_ sitemap item pointing to document 'aboutsubfolder' " +
                        "below the 'aboutfolder/aboutsubfolder' folder",
                        "common/aboutfolder/aboutsubfolder/aboutsubfolder", resolvedSiteMapItem.getRelativeContentPath());
