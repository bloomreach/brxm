...
public class HstComponentInvokerImpl implements HstComponentInvoker {
    ...
    public void setWebResourcesLocation(String webResourcesLocation) {
        if(webResourcesLocation == null) {
            log.info("No webResourcesLocation is configured. We take default location '{}' as webResourcesLocation", DEFAULT_WEB_RESOURCES_LOCATION);
            this.webResourcesLocation = DEFAULT_WEB_RESOURCES_LOCATION;
        } else if("/".equals(webResourcesLocation)) {
            log.info("A '/' as value for the webResourcesLocation is the same as an empty String: The web resources are taken relatively to the webapp. " +
