...
<div>
  <ul>
    <c:forEach var="category" items="${taxonomy.categories}">
      <li><a href="${currentUrl}/${category.path}">${category.name}</a>
        <ul>
          <c:forEach var="subcategory" items="${category.children}">
            <li>
              <a href="${currentUrl}/${subcategory.path}">${subcategory.name}</a>
              <ul>
                <c:forEach var="subsubcategory"
                  items="${subcategory.children}">
                  <li><a
                    href="${currentUrl}/${subsubcategory.path}">${subsubcategory.name}</a>
                    <ul>
                      <c:forEach var="subsubsubcategory"
                        items="${subsubcategory.children}">
                        <li><a
                          href="${currentUrl}/${subsubsubcategory.path}">${subsubsubcategory.name}</a>
                        </li>
                      </c:forEach>
                    </ul></li>
                </c:forEach>
