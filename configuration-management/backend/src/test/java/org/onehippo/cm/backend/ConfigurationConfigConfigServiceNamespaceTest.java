 ...
 */
public class ConfigurationConfigConfigServiceNamespaceTest extends BaseConfigurationConfigServiceTest {
    ...
    public void expect_adding_already_existing_namespace_to_be_allowed() throws Exception {
        ...
        assertFalse(getNamespacePrefixes().contains("test3"));
        session.getWorkspace().getNamespaceRegistry().registerNamespace("test3", "http://www.onehippo.org/test/nt/3.0");
    ...
    public void expect_namespace_removal_to_have_no_effect() throws Exception {
        ...
        assertTrue(getNamespacePrefixes().contains("test4"));
        ...
        assertTrue(getNamespacePrefixes().contains("test4"));
        assertEquals(getNamespaceURIForPrefix("test4"), "http://www.onehippo.org/test/nt/4.0");
    }
    ...
    public void expect_namespace_change_to_abort_with_exception() throws Exception {
        final String source
                ...
                + "  namespace:\n"
                + "  - prefix: test5\n"
                + "    uri: http://www.onehippo.org/test/nt/5.0";
        ConfigurationModel configurationModel = applyDefinitions(source);
        ...
        final String source2
                ...
                + "  namespace:\n"
                + "  - prefix: test5\n"
                + "    uri: http://www.onehippo.org/test/nt/5.0-changed";
        try {
            applyDefinitions(source2, configurationModel);
            fail("Should have thrown a RuntimeException");
        } catch (RuntimeException e) {
            assertEquals("Failed to process namespace definition defined in test-group/test-project/test-module-0 [string]: " +
                    "namespace with prefix 'test5' already exists in repository with different URI. " +
                    "Existing: 'http://www.onehippo.org/test/nt/5.0', target: 'http://www.onehippo.org/test/nt/5.0-changed'. " +
